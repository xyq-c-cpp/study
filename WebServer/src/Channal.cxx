...
void Channal::setWriteCb(EventCb&& cb) {
  writeCb_ = std::move(cb);
}

void Channal::setReadCb(EventCb&& cb) {
  readCb_ = std::move(cb);
}

void Channal::setErrorCb(EventCb&& cb) {
  errorCb_ = std::move(cb);
}

void Channal::setEvent(uint32_t event) {
  event_ = event;
}

void Channal::setMsg(std::shared_ptr<Message> msg) {
  msg_ = msg;
}

void Channal::setIsUpdateEvent(bool isUpdate) {
  isUpdate_ = isUpdate;
}
...
#ifdef DEBUG
      std::cout << "do nothing, and not timer, lastevent "
        << lastEvent_ << std::endl;
#endif
      return 0;
    }
...
void Channal::setErase(bool isErase) {
  isErase_ = isErase;
}

void Channal::setTimer(std::shared_ptr<Timer> timer) {
  timer_ = timer;
}
